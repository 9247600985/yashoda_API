...
export default class doctorChargeMasterController {
  ...
  constructor(private app: Router) {
    ...
    this.router.get(
      "/getClinics",
      authenticateToken,
      this.getClinics.bind(this),
    );
    this.router.get(
      "/getConsultationTypes",
      authenticateToken,
      this.getConsultationTypes.bind(this),
    );
    this.router.get(
      "/getDoctorDetails",
      authenticateToken,
      this.getDoctorDetails.bind(this),
    );
    this.router.post(
      "/getChargeDetails",
      authenticateToken,
      this.getChargeDetails.bind(this),
    );
    this.router.post(
      "/saveDoctorCharges",
      authenticateToken,
      this.saveDoctorCharges.bind(this),
    );
    this.router.get(
      "/getDoctorChargeList",
      authenticateToken,
      this.getDoctorChargeList.bind(this),
    );
  }
  ...
  async saveDoctorCharges(req: Request, res: Response): Promise<void> {
    ...
    const deleteSql = `DELETE FROM Mst_ChargeSheet_TM WHERE CLNORGCODE = @clinicId AND TARIFFID = @tariffId AND COUNTRY_ID = @countryId AND CURRENCY = @currency`;
    const insertSql = `INSERT INTO Mst_ChargeSheet_TM (Doctor_ID, IP_FollowUp_Visits, IP_FollowUp_Days, Valid_Days, Valid_Visits, FollowUp_Days, FreeFollowUp_No, PaidFollowUp_No, NewVisit_Charge, PaidFollowUp_Charge, IPFollowUp_Charge, RevisionId, CLNORGCODE, TARIFFID, COUNTRY_ID, CURRENCY, cons_typeid) VALUES (@doctorId, @ipVisits, @ipDays, @validDays, @validVisits, @freeDays, @freeVisits, @paidVisits, @newVisit, @paidFollowUp, @ipFollowUp, @revisionId, @clinicId, @tariffId, @countryId, @currency, @consType)`;
    try {
  ...
  async getDoctorChargeList(req: Request, res: Response): Promise<void> {
    const { countryId, clinicId, tariffId, currency } = req.query as Record<
      string,
      string
    >;
    const sql = `
