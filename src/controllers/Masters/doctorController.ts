...
//loop for parameters
  ...
  async getDoctorMaster(req: Request, res: Response): Promise<void> {
    const sql = `select isnull(REF_HOSPID,'') as REF_HOSPID, USERID,code,FirstName,MobileNo,DM.Status,S.Speciality_Name,md.DEPTNAME,DM.Qualification,DM.RegistrationNo,DM.TELECONS,VIDEOCONS,WALKINCONS,HomeVisitCons,INPATVISITCONS,EMERGENCYCONS,DM.SpecializationId,DM.department,Convert(Varchar(10),DM.VALID_UPTO,103)VALID_UPTO,CLINIC_CODE,DM.Country_Id,DM.EXPERIENCE,DM.LANGUAGES,DM.Hospital,DM.HospitalLocation,isnull(dm.Qualification1,'') as Qualification1,isnull(dm.Qualification2,'') as Qualification2,isnull(dm.Qualification3,'') as Qualification3,isnull(dm.Qualification4,'') as Qualification4 from Mst_DoctorMaster DM Left join Speciality_Master S on S.Speciality_ID = DM.SpecializationId Left join Mst_Department md on md.DEPTCODE = dm.Department order by FirstName`;
    try {
    ...
    try {
      ...
      for (const lang of DoctorData.DOCT_LANGUAGES || []) {
        await new sql.Request(transaction)
          ...
          .query(`
            INSERT INTO TM_DOCTORLANGUAGES
            (DOCT_ID, LANG_ID, STATUS, EDITED_BY, EDITED_ON)
      ...
      let updateQuery = `
        UPDATE Mst_DoctorMaster SET
          REF_HOSPID=@REF_HOSPID,
          USERID=@USERID,
          FirstName=@FirstName,
          MobileNo=@MobileNo,
          status=@status,
          ...
          WALKINCONS=@WALKINCONS,
          HOMEVISITCONS=@HOMEVISITCONS,
          INPATVISITCONS=@INPATVISITCONS,
          EMERGENCYCONS=@EMERGENCYCONS,
      ...
      request
        ...
        .input("SpecializationId", DoctorData.Speciality)
        .input("TELECONS", DoctorData.Ph_ch)
        .input("VIDEOCONS", DoctorData.Video_ch)
        .input("Department", DoctorData.Dept)
        .input("RegistrationNo", DoctorData.RegNo)
        .input("Qualification", DoctorData.Qualification)
        .input("WALKINCONS", DoctorData.Walkin_ch)
        .input("HOMEVISITCONS", DoctorData.HomeVisit_ch)
        .input("INPATVISITCONS", DoctorData.InpatVisit_ch)
        .input("EMERGENCYCONS", DoctorData.Emergency_ch)
        .input("VALID_UPTO", DoctorData.Valid_Upto)
        .input("CLINIC_CODE", DoctorData.Clinic)
        .input("Country_Id", DoctorData.Country)
        .input("EXPERIENCE", DoctorData.Experience)
 ...
 async  insertDoctorMaster(  req: Request,  res: Response): Promise<void> {
  try {
    ...
    const count = (Number(codeRes.records[0].Code) + 1)
      .toString()
      .padStart(6, "0");
    ...
    const dupSql = `
      ...
      WHERE FirstName=@FirstName
        ...
        AND TELECONS=@TELECONS
        AND VIDEOCONS=@VIDEOCONS
    ...
    const dupRes = await executeDbQuery(dupSql, {
      FirstName: d.NAME,
      MobileNo: d.Mobile,
      SpecializationId: d.Speciality,
      TELECONS: d.Ph_ch,
      VIDEOCONS: d.Video_ch,
      Department: d.Dept,
    ...
    await executeDbQuery(
      `DELETE FROM TM_DOCTORLANGUAGES WHERE DOCT_ID=@DOCT_ID`,
      { DOCT_ID: count }
    );
    ...
    for (const lang of d.DOCT_LANGUAGES || []) {
      await executeDbQuery(
        `INSERT INTO TM_DOCTORLANGUAGES
         (DOCT_ID, LANG_ID, STATUS, CREATED_BY, CREATED_ON)
         VALUES (@DOCT_ID,@LANG_ID,'A','',GETDATE())`,
        {
          DOCT_ID: count,
    ...
    await executeDbQuery(insertDoctorSql, {
      ...
      RegistrationNo: d.RegNo,
      Qualification: d.Qualification,
      WALKINCONS: d.Walkin_ch,
      HOMEVISITCONS: d.HomeVisit_ch,
      INPATVISITCONS: d.InpatVisit_ch,
      EMERGENCYCONS: d.Emergency_ch,
      SIGNATURE_PATH: sigPath,
      PROFILE_PATH: profilePath,
      VALID_UPTO: d.Valid_Upto,
      CLINIC_CODE: d.Clinic,
      Country_Id: d.Country,
    ...
    await executeDbQuery(
      `INSERT INTO Mst_UserDetails
       (userid, username, password, usertype, roles,
        created_by, created_on, status,
        hospitalname, clnorgcode, mobile, email, IMAGEPATH)
       VALUES
       (@userid,@username,@password,'N','003',
        @created_by,GETDATE(),'A',
