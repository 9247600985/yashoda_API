...
export default class sampleCollectionController {
    ...
    async getDetails1(req: Request, res: Response): Promise<void> {
        const input: any = (req.body && Object.keys(req.body).length ? req.body : req.query) || {};

        let query = '';

        // Map tables ONLY by labtypid (do not gate on labtypname/hospid)
        const labtypid = (input.labtypid ?? '').toString().trim();
        let TBL_ORDERMST = '';
        let TBL_ORDERTRN = '';
        let TBL_RESULTMST = '';
        let TBL_RESULTTRN = ''; // parity with C# (unused)
        ...
        const sampleStatusClause = (value: string): string => {
            ...
            return ''; // any other value → no extra clause (parity with C#)
        };
        ...
        if (CompanyId !== '') {
            ...
            if ((input.deptcode ?? '') !== '') {
                query += `
        select rm.RESULTNO, rm.RESULTDATE, rt.RESTEXT, ot.SAMLECOLNO,
               CASE WHEN OM.BILLNO='' THEN OT.ORDERNO ELSE OM.BILLNO END +', ' + ot.SAMLECOLNO+','+
               CASE WHEN (OT.RESULTNO='' OR OT.RESULTNO IS NULL) THEN ISNULL(OT.RESULTNO,'') ELSE RM.RESULTNO END +',  IP No:'+om.IPNO+', Patient Name:'+ om.PATNAME +',  OPD No:'+ ISNULL(om.opdregno,'') as Details,
               ot.ACCEPTEDON, ot.ACCEPTEDAT, om.MEDRECNO, dm.Firstname, om.ORDERNO, om.CLINICALNOTES, om.RACECD, ot.TESTCODE, tm.TESTNAME,
               ot.SAMCOLDATE, ot.RLTPANPROF, sm.SPECDESC
          from ${TBL_ORDERTRN} ot
          left join ${TBL_ORDERMST} om on ot.ORDERNO=om.ORDERNO
          left join DGL_TESTMASTER tm on ot.TESTCODE=tm.TESTCODE
          left join Mst_DoctorMaster dm on om.ORDEREDBY=dm.Code
          left join DGL_SPECIMENMAST sm on ot.SPECCODE=sm.SPECCODE
          left join DGL_ReportTypeMAST rt on tm.RESTYPECD=rt.RESCODE
          left join ${TBL_RESULTMST} rm on rm.RESULTNO=OT.RESULTNO and rm.SAMLECOLNO=ot.SAMLECOLNO
      ...
      `;
                ...
                query += `
        select distinct rm.RESULTNO, rm.RESULTDATE, rt.RESTEXT, ot.SAMLECOLNO,
               CASE WHEN OM.BILLNO='' THEN OT.ORDERNO ELSE OM.BILLNO END +',' + ot.SAMLECOLNO+', '+
               CASE WHEN (OT.RESULTNO='' OR OT.RESULTNO IS NULL) THEN ISNULL(OT.RESULTNO,'') ELSE RM.RESULTNO END +', IP No:'+om.IPNO+', Patient Name:'+ om.PATNAME +',  OPD No:'+ ISNULL(om.opdregno,'') as Details,
               ot.ACCEPTEDON, ot.ACCEPTEDAT,  om.MEDRECNO, dm.Firstname, om.ORDERNO, om.CLINICALNOTES, om.RACECD, ot.TESTCODE, tm.TESTNAME,
               ot.SAMCOLDATE, ot.RLTPANPROF, sm.SPECDESC
      ...
      `;
        ...
        select rm.RESULTNO, rm.RESULTDATE, rt.RESTEXT, ot.SAMLECOLNO,
          ...
          left join DGL_ReportTypeMAST rt on tm.RESTYPECD=rt.RESCODE
          left join ${TBL_RESULTMST} rm on rm.RESULTNO=OT.RESULTNO and rm.SAMLECOLNO=ot.SAMLECOLNO
          inner join IPD_ADMISSION IA on IA.IPNO=OM.IPNO
         where tm.LABDPTCODE=@deptcode
           and ot.ACCEPTEDON>=@fromdate and ot.ACCEPTEDON<=@todate
      `;
            } else {
      ...
      `;
            ...
            // No Company_Id: single SELECT (no OPD/IPD split and no fixed ORDERTYPE)
            if ((input.deptcode ?? '') !== '') {
                query = `
        select rm.RESULTNO, rm.RESULTDATE, rt.RESTEXT, ot.SAMLECOLNO,
               CASE WHEN OM.BILLNO='' THEN OT.ORDERNO ELSE OM.BILLNO END +', ' + ot.SAMLECOLNO+','+
      ...
      `;
        ...
        // Params (LIKE values wrapped in % to mirror C# AddWithValue pattern)
        const params = {
        ...
        try {
            ...
            const details = records.map((r: any) => ({
                CODE: r.Details ?? '',
                NAME: r.SAMLECOLNO ?? '',
                REGNO: fmtDate(r.ACCEPTEDON),  // mirrors GetDate() intention
                ADJDATE: r.ACCEPTEDAT ?? '',   // C# ToString()
                Value: r.TESTCODE ?? '',
                TESTNAME: r.TESTNAME ?? '',
                Firstname: r.Firstname ?? '',
                qty: r.RESTEXT ?? '',
                TRANNO: fmtDate(r.ACCEPTEDON), // mirrors GetDate() intention
                STATUS: r.ORDERNO ?? '',
                RESULTNO: r.RESULTNO ?? '',
            }));
