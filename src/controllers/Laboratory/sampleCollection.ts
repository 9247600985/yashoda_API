...
export default class sampleCollectionController {
  ...
  async getLabDepartments(req: Request, res: Response): Promise<void> {
    const sql = `SELECT LABDPTCODE, LABDPTDESC FROM DGL_LABDEPT WHERE STATUS = 'A' AND LABTYPE = '01' ORDER BY LABDPTDESC`;
    try {
  ...
  async showTest(req: Request, res: Response): Promise<void> {
    ...
    let SampleStatus = input.SampleStatus
      ? decodeURIComponent(input.SampleStatus)
      : "";
  ...
  async showData(req: Request, res: Response): Promise<void> {
    ...
    const Status = input.SampleStatus
      ? decodeURIComponent(input.SampleStatus)
      : "";
    const Priority = input.Priority || "";
    const SampleStatus = input.SampleStatus
      ? decodeURIComponent(input.SampleStatus)
      : "";
    ...
    if (SampleStatus) {
      ...
      if (
        ...
        cleanedParts.every((s) => s.toUpperCase() === "A")
      ) {
        normalised = "A";
    ...
    try {
      ...
      if (Company_Id) {
        ...
        if (SectionID) {
          ...
          if (Patient_type !== "A") sqlQuery += ` AND om.PATTYPE=@PATTYPE`;
  ...
  async updateDGLORDERTRN(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const { records: orderTrnRecords } = await executeDbQuery(
        testStatusQuery,
        testStatusParams,
        { transaction },
      );
      ...
      const { records: osRecords } = await executeDbQuery(
        orderStatusQuery,
        { ORDERNO: orderNo },
        { transaction },
      );
      ...
      const { records: fyRecords } = await executeDbQuery(
        finYearQuery,
        { CLNORGCODE: hospitalId },
        { transaction },
      );
      ...
      const { records: orderTypeRecords } = await executeDbQuery(
        orderTypeQuery,
        { SAMLECOLNO: sampleNo, CLNORGCODE: hospitalId },
        { transaction },
      );
      const orderType = orderTypeRecords.length
        ? orderTypeRecords[0].ORDERTYPE
        : null;
      ...
      if (orderType === "I") {
        ...
        const { records: profRecords } = await executeDbQuery(
          profileQuery,
          { SAMLECOLNO: sampleNo, CLNORGCODE: hospitalId },
          { transaction },
        );
        ...
        if (profileId && profOrderNo) {
          ...
          const { records: cntRecords } = await executeDbQuery(
            countQuery,
            { PROFID: profileId, ORDERNO: profOrderNo },
            { transaction },
          );
  ...
  async getDetails(req: Request, res: Response): Promise<void> {
    ...
    switch (labtypid) {
      ...
      default:
        res.status(400).json({
          status: 1,
          message: "Invalid labtypid. Expected 01/02/03/04.",
        });
        return;
    }
    ...
    const sampleStatusClause = (value: string): string => {
      ...
      if (value === "RE")
        return ` and  ot.SAMPLESTATUS in('RE') and rm.resultno like @ResultNo `;
      return "";
    };
    ...
    const optionalFiltersClause = (): string => {
      ...
      if ((input.priority ?? "") !== "")
        s += ` and ot.PRIORITY like @priority `;
