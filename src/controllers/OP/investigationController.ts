...
import {
  fetchCurrentFinYear,
  fetchCurrentNumber,
} from "../Masters/mastersController";
...
export default class investigationController {
  ...
  constructor(private app: Router) {
    ...
    this.router.get(
      "/loadRevision",
      authenticateToken,
      this.loadRevision.bind(this),
    );
    this.router.get(
      "/GetPaymentFields",
      authenticateToken,
      this.GetPaymentFields.bind(this),
    );
    this.router.get(
      "/GetDocFavDetails",
      authenticateToken,
      this.GetDocFavDetails.bind(this),
    );
    ...
    this.router.get(
      "/GET_FacilitySetUpDetails",
      authenticateToken,
      this.GET_FacilitySetUpDetails.bind(this),
    );
    this.router.get(
      "/getProfileServices",
      authenticateToken,
      this.getProfileServices.bind(this),
    );
    this.router.get(
      "/GetPACKAGEServices",
      authenticateToken,
      this.GetPACKAGEServices.bind(this),
    );
    this.router.get(
      "/GET_FACILITYSETUP_BEDCATG",
      authenticateToken,
      this.GET_FACILITYSETUP_BEDCATG.bind(this),
    );
    this.router.get(
      "/getServicesCost",
      authenticateToken,
      this.getServicesCost.bind(this),
    );
    this.router.get(
      "/LoadCrdauthLtr",
      authenticateToken,
      this.LoadCrdauthLtr.bind(this),
    );
    this.router.get(
      "/LoadCrdAuthDetServCode",
      authenticateToken,
      this.LoadCrdAuthDetServCode.bind(this),
    );
    this.router.get(
      "/LoadCompruletrn",
      authenticateToken,
      this.LoadCompruletrn.bind(this),
    );
    this.router.get(
      "/loadSplServCost",
      authenticateToken,
      this.loadSplServCost.bind(this),
    );
    this.router.get(
      "/GetAuthDetails",
      authenticateToken,
      this.GetAuthDetails.bind(this),
    );
    this.router.get(
      "/GetTrnCount",
      authenticateToken,
      this.GetTrnCount.bind(this),
    );
    this.router.get(
      "/GetSearchDetailsJson",
      authenticateToken,
      this.GetSearchDetailsJson.bind(this),
    );
    this.router.get(
      "/GET_TAB_SEARCH_DETAILS",
      authenticateToken,
      this.GET_TAB_SEARCH_DETAILS.bind(this),
    );
    this.router.get(
      "/GET_TAB_SEARCH_DETAILS_Main",
      authenticateToken,
      this.GET_TAB_SEARCH_DETAILS_Main.bind(this),
    );
    this.router.get(
      "/BindPrintOutpatientBillPage",
      authenticateToken,
      this.BindPrintOutpatientBillPage.bind(this),
    );
    this.router.get(
      "/getEmergencyPercentage",
      authenticateToken,
      this.getEmergencyPercentage.bind(this),
    );
    this.router.get(
      "/Get_AmbDetailsPrint",
      authenticateToken,
      this.Get_AmbDetailsPrint.bind(this),
    );
    this.router.get(
      "/ServeicePage",
      authenticateToken,
      this.BindPrintServeice.bind(this),
    );
    this.router.get(
      "/PrintOutpatientBillPage1",
      authenticateToken,
    ...
    this.router.post(
      "/saveInvestigation",
      authenticateToken,
      this.saveInvestigation.bind(this),
    );
    this.router.post(
  ...
  async LoadCompruletrn(req: Request, res: Response): Promise<void> {
    ...
    const params = {
      Strcompcode: input.Strcompcode,
      strUntiId: input.strUntiId,
      strrevisionid: input.strrevisionid,
    };
  ...
  async DisplayPrintOutpatientBillPagedispaly1(
    req: Request,
    res: Response,
  ): Promise<void> {
    const input = req.method === "GET" ? req.query : req.body;
  ...
  async saveInvestigation(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const params = {
        ...
        InvBillNo: BillNo,
        HospId: input.CLNORGCODE,
        RECEIPTNO: ReceiptNo,
        MedRecNo: input.MedRecNo,
        MedRecNo1: input.MedRecNo,
        Sal_code: input.Sal_code,
        PATNAME: input.PATNAME,
        PATNAME1: input.PATNAME,
        Age: input.Age,
        Gender: input.Gender,
        ...
        Doctor: input.Doctor,
        companycode_new: input.companycode_new,
        Autho_Letter: input.Autho_Letter,
        LetterValidDate: input.LetterValidDate,
        Tarif_ID: input.Tarif_ID,
        CovAmt: input.CovAmt,
        UnCovAmt: input.UnCovAmt,
        TotServAmt: input.TotServAmt,
        TotPatServAmt: input.TotPatServAmt,
        PatDiscAmt: input.PatDiscAmt,
        PatAmtPaid: input.PatAmtPaid,
        CompAmt: input.CompAmt,
        CompDisc: input.CompDisc,
        CompPaid: input.CompPaid,
        CrdAuthBy: input.CrdAuthBy,
        DiscAuthBy: input.DiscAuthBy,
        DrawnOn: input.DrawnOn,
        PatCatg: input.PatCatg,
        EMPCODE: input.EMPCODE || "",
        EMPCODE1: input.EMPCODE || "",
        DeptCode: input.DeptCode,
        NetAmt: input.NetAmt,
        TotPaidAmt: input.TotPaidAmt,
        TotalDiscount: input.TotalDiscount,
        PatType: input.PatType,
        IPNO: "",
        CREATED_BY: input.CREATED_BY,
        WardNo: input.WardNo,
        BedCatg: input.BedCatg,
        Revision: input.Revision,
        DiscCatg: input.DiscCatg,
        MedRecNo2: input.MedRecNo,
        refferalAgent: input.refferalAgent,
      };
      ...
      if (input.PayMode !== "003") {
        Insert_OPD_Receipts =
          "insert OPD_RECEIPTS (CLNORGCODE,FINYEAR,MEDRECNO,CNTRCODE, RCPTTYPE,RECEIPTNO,RECEIPTDATE,OPDBILLNO,PAYMODE,CHEQUEDDNO,CHEQUEDATE,BANKNAME, RCVDFROM,REMARKS,AMOUNT,DEPTCODE,CREATED_BY,CREATED_ON,STATUS,CNNO,PC_Code,TARIFFID,REVISIONID,OPDREGNO) values (@CLNORGCODE, @FINYEAR, @MedRecNo, @CashCounter, 'OB', @RECEIPTNO, GetDate(), @InvBillNo, @PayMode, @CheqNo, @CheqDate, @BankName, @CREATED_BY, @Remarks, @TotPaidAmt, '',@CREATED_BY,GETDATE(),'A','', @PatCatg, @Tarif_ID, @Revision, @OPDREGNO)";

        const OPD_Receipts_Params = {
          CLNORGCODE: input.CLNORGCODE,
          FINYEAR: FinYear,
          MedRecNo: input.MedRecNo,
          CashCounter: input.CashCounter,
      ...
      const hasLab = rows.some(
        (r: { ServType: string }) => r.ServType === "01",
      );
      const hasRad = rows.some(
        (r: { ServType: string }) => r.ServType === "02",
      );
      const hasCard = rows.some(
        (r: { ServType: string }) => r.ServType === "03",
      );
      const hasSurg = rows.some(
        (r: { ServType: string }) => r.ServType === "04",
      );

      if (hasLab) {
        Insert_DGL_ORDERMST =
          "insert DGL_ORDERMST (CLNORGCODE,FINYEAR,ORDERNO,ORDERDATE,ORDERTIME,ORDERTYPE, PATTYPE,IPNO,MEDRECNO,PATNAME,AGE,SEX,BILLNO,NURSTCODE,REPORTTO, ORDEREDBY,STARTDATE,STARTTIME,STOPDATE,STOPTIME,BEDCATGCD,TARIFFID, CLINICALNOTES,REPAFTER,REMARKS,ORDER_STATUS,RACECD,LABTYPECD,LABTYPE, WARDNO,BEDNO,CLINICCODE,REQUESTNO,CREATED_BY,CREATED_ON,EDITED_BY, EDITED_ON,STATUS,PRIORITY,PATCATG,RoomNo,OPDREGNO) values (@CLNORGCODE, @FINYEAR, @labOrderCount, GetDate(),CONVERT(VARCHAR(8),GETDATE(),108),'O', @PatType, @IPNO, @MedRecNo, @PATNAME, @Age, @Gender,@pBillNO,'', @Doctor, @Doctor1,GETDATE(),CONVERT(VARCHAR(8),GETDATE(),108),'1900-01-01','00:00:00.0000000', @BedCatg, @Tarif_ID,'','', @Remarks,'OP','', @ServType,'', @WardNo,  @BedCatg1,'','',@CREATED_BY, GETDATE(),'','','A','R', @PatCatg, @WardNo1, @OPDREGNO)";

        const Insert_DGL_ORDERMST_params = {
          CLNORGCODE: input.CLNORGCODE,
          FINYEAR: FinYear,
          labOrderCount: LABOrderNo,
          PatType: input.PatType,
          IPNO: input.IPNO,
          MedRecNo: input.MedRecNo,
          PATNAME: input.PATNAME,
          ...
          pBillNO: BillNo,
          Doctor: input.Doctor,
          Doctor1: input.Doctor,
          BedCatg: input.BedCatg,
          Tarif_ID: input.Tarif_ID,
          Remarks: input.Remarks,
          ServType: "01",
          WardNo: input.WardNo,
          BedCatg1: input.BedCatg,
          CREATED_BY: input.CREATED_BY,
          PatCatg: input.PatCatg,
          WardNo1: input.WardNo,
          OPDREGNO: input.OPDREGNO,
        };

        const rowsAffected = await executeDbQuery(
          Insert_DGL_ORDERMST,
          Insert_DGL_ORDERMST_params,
          { transaction },
        );
      }
      ...
      if (hasRad) {
        ...
        const insert_DGR_ORDERMST_params = {
          CLNORGCODE: input.CLNORGCODE,
          FINYEAR: FinYear,
          radOrderCount: RADOrderNo,
          PatType: input.PatType,
          IPNO: input.IPNO,
          MedRecNo: input.MedRecNo,
          PATNAME: input.PATNAME,
      ...
      for (const row of rows) {
        ...
        const Insert_OPD_BILLTRN_params = {
          ...
          CompAmt: row.CompAmt,
          PatDisc: row.PatDisc,
          CompDisc: row.CompDisc,
          Remarks: input.Remarks,
          Dept: row.Dept,
          ServGrpCode: row.ServGrpCode,
          ServSubGrpCode: row.ServSubGrpCode,
          ...
          Amount: row.Amount,
          RepDoc: row.RepDoc,
          DocShare: row.DocShare,
          HospShare: row.HospShare,
          CREATED_BY: input.CREATED_BY,
          DiscPer: row.DiscPer,
          revissionId: row.revissionId,
        ...
        if (row.ServType == "04") {
          ...
          const rowsAffected1 = await executeDbQuery(
            INSERT_DGS_ORDERTRN,
            INSERT_DGS_ORDERTRN_params,
            { transaction },
          );
        }
        ...
        if (row.ServType == "01") {
          ...
          const Insert_DGL_ORDERTRN_params = {
            ...
            ServSubGrpCode: row.ServSubGrpCode,
            labOrderCount: LABOrderNo,
            splServCode: row.splServCode || row.ServCode,
            PROFILEYN: row.PROFILEYN || "N",
            PROFILEID: row.PROFILEID || "",
            DGL_Seq: DGL_Seq,
            SplServCode1: row.SplServCode || row.ServCode,
            Tarif_ID: input.Tarif_ID,
            BedCatg: input.BedCatg,
            Qty: row.Qty,
            Rate: row.Rate,
        ...
        if (row.ServType == "03") {
          ...
          const Insert_DGC_ORDERTRN_params = {
            CLNORGCODE: input.CLNORGCODE,
            FINYEAR: FinYear,
            ServType: row.ServType,
            ServGrpCode: row.ServGrpCode,
        ...
        if (row.ServType == "02") {
          ...
          const INSERT_DGR_ORDERTRN_params = {
            ...
            splServCode: row.splServCode || row.ServCode,
            DGR_Seq: DGR_Seq,
            splServCode1: row.ServCode || row.splServCode,
            Tarif_ID: input.Tarif_ID,
            BedCatg: input.BedCatg || "",
            Qty: Number(row.Qty ?? 1),
            Rate: Number(row.Rate ?? 0),
            Rate1: Number(row.Rate ?? 0),
            DGR_Seq1: DGR_Seq,
          };

          const rowsAffected = await executeDbQuery(
            INSERT_DGR_ORDERTRN,
            INSERT_DGR_ORDERTRN_params,
            { transaction },
          );
        }
      }
      ...
      Update_Patient_Master =
        "update patient_master set age= @Age, Patient_DOB= @patDOB, Comp_Id= @companycode_new where patientmr_no= @MedRecNo";

      const Update_Patient_Master_params = {
        Age: input.Age,
        patDOB: input.patDOB,
        companycode_new: input.companycode_new,
        MedRecNo: input.MedRecNo,
  ...
  async GetSearchDetailsJson(req: Request, res: Response): Promise<void> {
    const input = req.method === "GET" ? req.query : req.body;
    ...
    try {
      ...
      let sql = `select u.USERNAME, c.Name companyName, op.Billno, op.Billdate,
                   op.OPDREGNO, op.MEDRECNO, op.PATNAME, op.AGE, op.SEX,
                   op.NetAmount, op.PATAMTPAID,
                   (op.NETAMOUNT - op.AMOUNTPAID) DueAmt,
                   op.CREATED_BY, op.DOCTCD, dd.Firstname,
                   op.ipno, op.AMOUNTPAID, op.status
                   from OPD_BILLMST op
                   left join Mst_DoctorMaster dd on dd.Code = op.DOCTCD
                   left join Company c on c.Com_Id = op.CRDCOMPCD
                   left join Mst_UserDetails u on u.USERID = op.CREATED_BY
                   where BILLTYPE='OB'
                   and CONVERT(VARCHAR(10), BILLDATE, 120) BETWEEN @fromdate AND @todate`;

      const params: any = { fromdate: fromDate, todate: toDate };

      // Only filter by clinic if it was actually provided
      if (hospid) {
        sql += " AND OP.CLNORGCODE = @hospid";
      ...
      const data = records.map((r: any) => ({
        Billno: r.Billno ?? "",
        Billdate: formatDateChange(r.Billdate) ?? "",
        MEDRECNO: r.MEDRECNO ?? "",
        PATNAME: r.PATNAME ?? "",
        companyName: r.companyName ?? "",
        AGE: r.AGE ?? "",
        SEX: r.SEX ?? "",
        NetAmount: parseFloat(r.NetAmount ?? 0).toFixed(2),
        PATAMTPAID: parseFloat(r.PATAMTPAID ?? 0).toFixed(2),
  ...
  async GET_TAB_SEARCH_DETAILS_Main(
    req: Request,
    res: Response,
  ): Promise<void> {
    const input = req.method === "GET" ? req.query : req.body;
  ...
  async BindPrintOutpatientBillPage(
    req: Request,
    res: Response,
  ): Promise<void> {
    const input = req.method === "GET" ? req.query : req.body;
    ...
    try {
      ...
      const details = records.map((r: any) => {
        ...
        return {
          ...
          MEDRECNO: r.MEDRECNO,
          BILLDATE: r.BILLDATE
            ? moment(r.BILLDATE).format("DD/MM/YYYY HH:mm:ss")
            : "",
          PATNAME: r.PATNAME,
          AGE: r.AGE,
          SEX: r.SEX,
          ...
          CHEQUEDATE: r.CHEQUEDATE
            ? moment(r.CHEQUEDATE).format("DD/MM/YYYY")
            : "",
          DRAWNON: r.DRAWNON,
          Name: r.Name,
