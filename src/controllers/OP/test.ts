...
async AccountReport11(req: Request, res: Response): Promise<void> {
  const input: any = req.method === "GET" ? req.query : req.body;
  ...
  let paymode = String(input.paymode || "").trim();
paymode=`%${paymode}%`;
  const clinicCode = String(input.Clinic_Code || "").trim();
...
const clinicFilter = clinicCode
  ...
  : ` `;
   ...
   sql = `SELECT  DocNo,Date, CashBankAC, Division,MODE_OF_PAYMENT,sNarration,CHEQUE_NO, CHEQUE_DT, Account,Amount,REMARKS FROM (
    SELECT 
       CASE  WHEN OH.PAYMODE = '001'  THEN
    
      'MA1/24-25/MCR-' + 
             CAST(ROW_NUMBER() OVER (
                 PARTITION BY CASE WHEN OH.PAYMODE  IN ('001') THEN 1 END
                 ORDER BY CONVERT(date, OH.BILLDATE)--, T.ACC_CLINIC_NAME_CASH
             ) AS VARCHAR)
    
        WHEN OH.PAYMODE ='002' THEN ''
        ELSE 'MA1/24-25/YHCL-' + 
             CAST(ROW_NUMBER() OVER (
                 PARTITION BY CASE WHEN OH.PAYMODE NOT IN ('001','002') THEN 1 END
                 ORDER BY CONVERT(date, OH.BILLDATE)--, T.ACC_CLINIC_NAME_CASH
             ) AS VARCHAR)
    END AS DocNo
    
    , CONVERT (varchar(20), OH.BILLDATE, 103) Date,
    'MAIN CASH - ' + T.ACC_CLINIC_NAME_CASH CashBankAC,'MA1' Division
    , CASE WHEN OH.PAYMODE = '001' THEN 'CAS' WHEN OH.PAYMODE = '002' THEN 'CHQ' WHEN PAYMODE IN('004', '005', '006')
    THEN 'CRD' ELSE 'UPI' END MODE_OF_PAYMENT
    
    ,A.ACCREM + T.ACC_CLINIC_NAME_CASH + '(' + CONVERT(varchar(20), OH.BILLDATE, 103) + ')' sNarration
    
    ,'' CHEQUE_NO,'' CHEQUE_DT,A.ACCODE Account
    , ROUND(COALESCE(SUM(OD.AMOUNT - (OD.SERDISCOUNT + OD.PATCNAMT)), 0), 0) Amount
    ,'' Reference,'' REMARKS
     FROM OPD_BILLMST OH
    LEFT JOIN OPD_BILLTRN OD ON OH.BILLNO = OD.BILLNO
    LEFT JOIN MST_SERVICES S ON OD.SERVCODE = S.SERVCODE
    LEFT JOIN MST_ACCOUNTS A ON A.ACCODE = S.ACCODE
    LEFT JOIN TM_CLINICS T ON T.CLNORGCODE = OD.CLNORGCODE
    WHERE OH.PAYMODE =${paymode}
    ${commonDateOrBillFilter}
    ${clinicFilter}
 
    GROUP BY CONVERT(varchar(20), OH.BILLDATE, 103),T.ACC_CLINIC_NAME_CASH,
    A.ACCREM,A.ACCODE 
    ,OH.PAYMODE
    ,OH.BILLDATE
    HAVING ROUND(COALESCE(SUM(OD.AMOUNT - (OD.SERDISCOUNT + OD.PATCNAMT)), 0), 0) <> 0
    ...
    ORDER BY   TRY_CAST(RIGHT(DocNo, CHARINDEX('-', REVERSE(DocNo)) - 1) AS INT)`;
...
async AccountReport1(req: Request, res: Response): Promise<void> {
  const input: any = req.method === "GET" ? req.query : req.body;
  ...
  // ✅ No longer adding %% here — handled in params below
  const paymode = String(input.paymode || "").trim();
  const clinicCode = String(input.Clinic_Code || "").trim();
  ...
  // ✅ Use @Paymode parameter instead of direct interpolation
  const sql = `
    SELECT DocNo, Date, CashBankAC, Division, MODE_OF_PAYMENT, sNarration,
           CHEQUE_NO, CHEQUE_DT, Account, Amount, REMARKS
    FROM (
      SELECT
        CASE
          WHEN OH.PAYMODE = '001' THEN
            'MA1/24-25/MCR-' +
            CAST(ROW_NUMBER() OVER (
              PARTITION BY CASE WHEN OH.PAYMODE IN ('001') THEN 1 END
              ORDER BY CONVERT(date, OH.BILLDATE)
            ) AS VARCHAR)
          WHEN OH.PAYMODE = '002' THEN ''
          ELSE 'MA1/24-25/YHCL-' +
            CAST(ROW_NUMBER() OVER (
        ...
        END AS DocNo,
        CONVERT(varchar(20), OH.BILLDATE, 103) Date,
        'MAIN CASH - ' + T.ACC_CLINIC_NAME_CASH CashBankAC,
        'MA1' Division,
        CASE
          WHEN OH.PAYMODE = '001' THEN 'CAS'
          WHEN OH.PAYMODE = '002' THEN 'CHQ'
          WHEN OH.PAYMODE IN ('004', '005', '006') THEN 'CRD'
          ELSE 'UPI'
        END MODE_OF_PAYMENT,
        A.ACCREM + T.ACC_CLINIC_NAME_CASH + '(' + CONVERT(varchar(20), OH.BILLDATE, 103) + ')' sNarration,
        '' CHEQUE_NO, '' CHEQUE_DT,
      ...
      LEFT JOIN MST_ACCOUNTS A ON A.ACCODE = S.ACCODE
      LEFT JOIN TM_CLINICS T ON T.CLNORGCODE = OD.CLNORGCODE
      WHERE OH.PAYMODE LIKE @Paymode   -- ✅ parameterized, with LIKE for wildcard support
      ${commonDateOrBillFilter}
      ${clinicFilter}
      GROUP BY
        CONVERT(varchar(20), OH.BILLDATE, 103),
  ...
  try {
    ...
    const { records } = await executeDbQuery(sql, {
      ...
      Bill_number: `%${billNo}%`,
      Clinic_Code: `%${clinicCode}%`,
      Paymode: `%${paymode}%`,   // ✅ wildcard added here in params, not in variable
      Offset: offset,
      PageSize: pageSize,
    });
  ...
  async DeptWiseReportForAccounts(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      SELECT
        ...
        OH.PATFNAME+' ' +OH.PATMNAME+' '+OH.PATLNAME AS Patientname,
