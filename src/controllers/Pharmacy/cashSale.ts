    ...
    try {
      ...
      const masterSql = `
        INSERT INTO INV_CASHSALEMST (
          ...
          COMAMTPAID, TOTPAIDAMOUNT, PAYMODE, MULTIPAYMENT, CREATED_BY, CREATED_ON, STATUS,CNAmount,CMPRCVD
        ) VALUES (
          ...
          0.00, @PAIDAMOUNT, @PAYMODE, @MULTIPAYMENT, @CREATED_BY, GETDATE(), 'A', 0.00,0.00
        )
      `;
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (mtrlCode && batchNo) {
          const updateStockSql = `
            UPDATE INV_STOCKMST 
            SET   TOTCASHSALE = ISNULL(TOTCASHSALE, 0) + @QTY
